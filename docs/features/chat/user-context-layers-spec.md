...
이 문서는 `docs/features/` 하위의 기능 명세에 해당하며, 채팅·AI 파이프라인과 연동되는 설계이다.
...
- 프로젝트 문서 계층: `docs/core/`, `docs/features/`, `docs/roadmap/` 등에 정의된 구조를 따른다.
- DB/마이그레이션: [CRITICAL: DATABASE INTEGRITY RULE], [MANDATORY BACKUP PROCEDURE]를 준수한다.
- 채팅·AI 파이프라인: `docs/features/chat/interrupt-strategy.md`, LangGraph·Gemini 연동 명세와 함께 참고하여 통합한다.
