...
- `app/lib/logger.server.ts`에 음성 관련 로그 추가
- 통화 세션별 크레딧 사용량 추적
- 에러 발생 시 상세 로그 기록
...
1. **오디오 파일 최적화**:
   - Cloudinary를 통한 자동 압축 및 포맷 변환
   - WebM/Opus 포맷 사용 (최적 압축률)

2. **캐싱**:
   - 자주 사용되는 TTS 응답 캐싱 (예: 인사말, 공통 응답)

3. **스트리밍 최적화**:
   - 청크 크기 최적화 (WebSocket 메시지 크기)
   - 버퍼링 최소화
...
3. **크레딧 보호**:
   - 서버 사이드에서만 크레딧 차감
   - 중복 차감 방지 (idempotency key)

## 12. 비즈니스 가치

### 12.1 사용자 경험 개선

*   **몰입도 증대**: 캐릭터와 실제 친구처럼 전화하는 경험 제공
*   **체류 시간 증가**: 텍스트 채팅보다 훨씬 긴 시간 동안 깊은 유대감을 형성
*   **접근성 향상**: 시각 장애인 또는 다중 작업 중인 사용자에게 유용

### 12.2 수익 모델

*   **프리미엄 서비스**: 고품질 통화 기능을 구독 모델(PREMIUM/ULTIMATE)의 핵심 가치로 제공
*   **크레딧 판매**: 음성 기능 사용을 위한 추가 크레딧 충전 유도
*   **차별화**: 경쟁사 대비 고급 기능 제공으로 프리미엄 포지셔닝

### 12.3 예상 수익 영향

**가정**:
- PREMIUM 등급 사용자 중 30%가 월 60분 통화 사용
- ULTIMATE 등급 사용자 중 50%가 월 300분 통화 사용
- 통화 사용자 중 20%가 추가 크레딧 충전 (월 평균 $10)

**예상 효과**:
- PREMIUM 등급 전환율 20% 증가
- ULTIMATE 등급 전환율 30% 증가
- 월간 추가 수익: 크레딧 충전으로 인한 $5,000-10,000 (사용자 수에 따라 변동)
...
### Phase 1 (우선순위: 높음)
- **기간**: 2-3주
- **목표**: 비동기 보이스 메시지 기능 구현
- **비즈니스 가치**: 즉시 사용 가능한 프리미엄 기능 제공

### Phase 2 (우선순위: 중간)
- **기간**: 4-6주
- **목표**: 실시간 통화 기능 구현
- **비즈니스 가치**: 최고 수준의 몰입 경험 제공

### Phase 3 (우선순위: 낮음)
- **기간**: 2-3주
- **목표**: 감정 기반 반응형 통화 구현
- **비즈니스 가치**: 차별화된 고급 기능 제공

**총 예상 개발 기간**: 8-12주 (병렬 작업 시 단축 가능)
