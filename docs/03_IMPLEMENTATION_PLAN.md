...
- [ ] 채팅 목록 화면의 'Add' (+) 버튼을 통한 새 대화방 생성 기능 구현
- [ ] 캐릭터 프로필 선택 시 즉시 1:1 대화방 이동 및 신규 세션 생성 로직
- [ ] 대화 삭제 기능: 메시지 DB 레코드와 함께 Cloudinary 원본 미디어 파일 완전 삭제 로직 구현
- [ ] 대화 초기화(Reset) 기능: 메시지 내역 삭제 및 AI의 장기 기억(User.bio.memory) 초기화 공정
