...
본 문서는 **USER_CONTEXT_LAYERS_SPEC**(`docs/features/chat/user-context-layers-spec.md`)을 바탕으로 한 단계별 구현 계획이다. 명세서의 5계층(memory, heartbeat, identity, soul, tools), 스키마, API, 보안, 마이그레이션을 실제 작업 단위로 나누어 진행 순서와 검증 기준을 정의한다.
...
| 문서 | 경로 | 용도 |
|------|------|------|
| 명세서 | `docs/features/chat/user-context-layers-spec.md` | 요구사항, 스키마, API, 보안 정의 |
| DB 무결성 규칙 | AGENTS.md | 백업 후 마이그레이션 원칙 |
| 인터럽트 전략 | `docs/features/chat/interrupt-strategy.md` | 채팅 파이프라인 연동 참고 |
| Phase 0 산출물 | `docs/roadmap/user-context-layers-phase0-findings.md` | User.bio 사용처, 등급 위치, LangGraph 노드, 캐릭터 목록, bio 포맷 |
...
**Phase 0 산출물**: 위 항목의 조사 결과는 `docs/roadmap/user-context-layers-phase0-findings.md`에 정리한다.
...
| 10.4 | 기능 문서 업데이트 | `docs/features/chat/` 또는 AI 파이프라인 문서에 "5계층 주입 위치", "토큰 예산 정책" 요약 추가 | 문서 |
