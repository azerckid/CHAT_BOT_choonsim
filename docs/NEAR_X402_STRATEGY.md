...
> **작업 철학**: UI/UX를 먼저 완성하고, 그 다음 백엔드 기능을 연결하는 방식으로 진행합니다. 이를 통해 사용자 경험을 먼저 검증하고, 기술적 복잡성을 나중에 처리할 수 있습니다.
...
- [ ] **Allowance 시스템 연동**
  ...
  - 한도 내 자동 결제 로직 구현
- [ ] **X402 인터셉터 구현**
  - 클라이언트 사이드 402 인터셉터 구현
  - 서버 사이드 402 응답 생성 로직 구현
  - 한도 내 자동 결제 플로우 구현
...
- **Phase 2**: 완전히 작동하는 UI (목업 API)
- **Phase 3**: 데이터베이스 스키마 및 기본 API
- **Phase 4**: 실제 작동하는 전체 시스템
