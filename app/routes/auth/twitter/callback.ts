...
/**
 * [AUTH_PLAN.md 2.1 준수] Twitter Callback Receiver
 * 역할: 트위터가 던져준 'code'와 'state'를 받아서
 * Better Auth 엔진이 이해할 수 있는 내부 경로로 전달합니다.
 */
export async function loader({ request }: LoaderFunctionArgs) {
    const url = new URL(request.url); // 예: .../auth/twitter/callback?code=abc&state=xyz
    
    // 핵심 수정: 사용자님의 경로를 라이브러리 내부 경로로 맵핑
    // 쿼리 파라미터(code, state)는 그대로 유지됩니다.
    url.pathname = "/auth/callback/twitter";
